...
Overview는 `review_queue`의 `adjustment` 항목(현금·레이어·종목의 비중 조정
정책 검토)을 먼저 보여주고, 현금/투자자산 비중 밴드와 투자 원금·현재 평가금·
...
정책 재검토 후보의 시장·기술 근거 생산기는 현재 축소된 핵심 검사 범위에
포함되지 않습니다. 따라서 API와 CLI는 후보 상태를 `not_implemented`로
명시하며, 후보 차이가 활성 정책 평가·Review Queue·정책 저장/활성화를
바꾸지 않습니다.
