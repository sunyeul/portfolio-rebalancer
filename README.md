...
```bash
uv run ips-pilot agent-brief --text "VOO 40
QQQ 60"
uv run ips-pilot agent-brief --snapshot-id 14
uv run ips-pilot diagnose --snapshot-id 14
uv run ips-pilot dca-plan --snapshot-id 14
...
Codex의 기본 진입점은 `agent-brief`입니다. 더 좁은 점검에는 `diagnose`, `dca-plan`, `review-queue`, `risk`를 사용합니다. 출력은 `ips_status`, `risk_flags`, `review_queue`, `dca_plan`, `playbook`, `guardrails` 순서로 읽고, CLI 결과를 매수/매도 주문 지시로 표현하지 않습니다. `evaluate`는 에이전트 판단용 호환 출력이 아니라 전체 분석/평가 원자료와 CSV 산출물 생성을 위한 raw 명령입니다. 에이전트 판단에는 `evaluate`의 `proposal.should_execute`, `buy_list`, `sell_list`를 직접 해석하지 말고 `agent-brief`와 목적별 명령의 DCA Plan/Review Queue를 사용합니다. 에이전트용 사용 규칙은 [.agents/skills/ips-pilot-cli-review/SKILL.md](.agents/skills/ips-pilot-cli-review/SKILL.md)에 둡니다.
