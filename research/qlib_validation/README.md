...
예측은 Qlib `StaticDataLoader`를 통과한 입력에 Qlib 네이티브
`LinearModel(estimator='ridge')`를 적용한다. `explanation.json`에는 최종 절편과 각
특징의 계수를 기록한다. 예측 대상은 종목별 현지 통화 종가 기준의 20거래일 수익률이며,
