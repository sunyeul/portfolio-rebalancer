...
class AssetMetrics(BaseModel):
    ...
    data_start: str | None = Field(None, description="유효 가격 시작일")
    data_end: str | None = Field(None, description="유효 가격 종료일")
    observation_count: int | None = Field(None, description="유효 가격 관측수")
    missing_ratio: float | None = Field(None, description="원본 가격 결측률")
...
class ProposalRow(BaseModel):
    ...
    ips_fit_score: float | None = Field(None, description="IPS 적합도")
    ips_fit_band: str | None = Field(None, description="IPS 등급")
    ips_score_role: float | None = Field(None, description="IPS 점수: 역할")
    ips_score_allocation: float | None = Field(None, description="IPS 점수: 비중")
    ips_score_thesis: float | None = Field(None, description="IPS 점수: 투자 논리")
    ips_score_risk: float | None = Field(None, description="IPS 점수: 위험")
    ...
    efficiency_warning: bool = Field(False, description="효율 점수 경고 여부")
...
class IPSActionRow(BaseModel):
    ...
    execution_type: str | None = Field(None, description="실행 유형")
    decision_context: str | None = Field(None, description="판단 모드")
    decision_summary: str | None = Field(None, description="판단 요약")
    decision_reasons: List[str] = Field(default_factory=list, description="판단 사유")
    risk_notes: List[str] = Field(default_factory=list, description="위험 메모")
