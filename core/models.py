...
class ProposalRow(BaseModel):
    ...
    target_preference_score: float | None = Field(None, description="목표 비중 선호 총점")
    target_score_efficiency: float | None = Field(None, description="목표 점수: 효율")
    target_score_risk: float | None = Field(None, description="목표 점수: 위험")
    target_score_thesis: float | None = Field(None, description="목표 점수: 투자 논리")
    target_score_data_quality: float | None = Field(None, description="목표 점수: 데이터 품질")
    group: str = Field("unclassified", description="IPS 고정 그룹 분류")
    dca_enabled: bool = Field(True, description="정기매수 조정 대상 여부")
    thesis_status: str = Field("unknown", description="투자 논리 상태")
